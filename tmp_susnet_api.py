...
MESH_PREFS_PATH = Path("/var/lib/susnet/meshtastic_prefs.json")

_BASE_DIR = Path(__file__).resolve().parent
_PROJECT_ROOT: Optional[Path] = None
for candidate in [_BASE_DIR] + list(_BASE_DIR.parents):
    if (candidate / ".git").exists() and (candidate / "Troubleshooting").exists():
        _PROJECT_ROOT = candidate
        break

_REPO_TROUBLE = Path("/home/gabe0000/susnet-private/Troubleshooting")
if _REPO_TROUBLE.exists():
    TROUBLE_ROOT = _REPO_TROUBLE
elif _PROJECT_ROOT is not None:
    TROUBLE_ROOT = _PROJECT_ROOT / "Troubleshooting"
else:
...
class ChannelPrefItem(BaseModel):
    mute_msg: bool = False
    mute_tts: bool = False
    solo_msg: bool = False
    solo_tts: bool = False
...
class MeshPrefsPayload(BaseModel):
    channels: Optional[Dict[str, ChannelPrefItem]] = None
    profiles: Optional[Dict[str, ProfilePayload]] = None
...
def _load_mesh_prefs() -> Dict[str, Any]:
    if not MESH_PREFS_PATH.exists():
        return {"channels": {}, "profiles": {}}
    try:
        data = json.loads(MESH_PREFS_PATH.read_text())
...
def _save_mesh_prefs(prefs: Dict[str, Any]) -> Dict[str, Any]:
    prefs.setdefault("channels", {})
    prefs.setdefault("profiles", {})
    MESH_PREFS_PATH.parent.mkdir(parents=True, exist_ok=True)
    MESH_PREFS_PATH.write_text(json.dumps(prefs, indent=2, sort_keys=True))
    return prefs
    ...
    if MESHTASTIC_JSON.exists():
        try:
            ...
            for entry in data[-200:]:
                ...
                channel = entry.get("channel") or "Primary"
                prof = profiles.get(from_id) if isinstance(profiles, dict) else {}
                nickname = prof.get("nickname") if isinstance(prof, dict) else None
                items.append({
                    "timestamp": ts,
                    "from": from_id,
                    "from_name": nickname or name_map.get(from_id, from_id),
                    "to": to_id,
...
def meshtastic_send(req: SendRequest):
    ...
    dest = req.dest
    cmd = [MESHTASTIC_CLI, "--sendtext", text]
    if dest:
        cmd.extend(["--dest", dest])
    try:
        _run(cmd)
    except HTTPException:
        # Fallback to queue file used by listener
        _append_queue_line(f"SEND:{text}")
    return {"ok": True}
...
@app.get("/api/meshtastic/prefs")
def meshtastic_prefs():
    return {"ok": True, "prefs": _load_mesh_prefs()}


@app.post("/api/meshtastic/prefs")
def meshtastic_prefs_update(req: MeshPrefsPayload):
    prefs = _load_mesh_prefs()
    if req.channels:
        channels = prefs.setdefault("channels", {})
        for ch, val in req.channels.items():
            channels[ch] = val.dict()
    if req.profiles:
        profiles = prefs.setdefault("profiles", {})
        for node_id, prof in req.profiles.items():
            profiles[node_id] = prof.dict()
    prefs = _save_mesh_prefs(prefs)
...
def meshtastic_topology():
    ...
    if raw_info:
        seen_ch = set()
        # Prefer channel names inside the JSON-ish info output
        for match in re.finditer(r'"name":\s*"([^"]+)"', raw_info):
            name = match.group(1).strip()
            if name and name.lower() not in ("true", "false") and name not in seen_ch:
                seen_ch.add(name)
                channels.append(name)
        if not channels:
            # Fallback: any line containing "Channel" with a colon
            for line in raw_info.splitlines():
                line_stripped = line.strip()
                if (line_stripped.startswith("Channel") or line_stripped.startswith("Index")) and ":" in line:
                    ch = line.split(":", 1)[1].strip()
                    if ch and ch.lower() not in ("true", "false") and ch not in seen_ch:
                        seen_ch.add(ch)
                        channels.append(ch)
    return {"ok": True, "nodes": nodes, "channels": channels}
...
def aprs_messages():
    ...
    return {"ok": True, "lines": filtered[:200], "config": cfg}
