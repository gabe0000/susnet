...
app = FastAPI(title="susnet-module-gmrshub", version="0.1.0")
...
@app.get("/health")
def health():
    return {"ok": True, "service": "gmrshub", "v1_base": V1_BASE_URL}
...
def inbound_health(node: Optional[str] = Query(None)):
    ...
    return _get("/api/gmrshub/inbound-health", params)
...
def inbound_test_window(payload: Dict[str, Any] = Body(default={})):
    ...
    if isinstance(payload, dict):
        try:
            ...
            req_timeout = TIMEOUT
    return _post("/api/gmrshub/inbound-test-window", payload, timeout=req_timeout)
