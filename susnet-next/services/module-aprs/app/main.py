...
app = FastAPI(title="susnet-module-aprs", version="0.1.0")
...
@app.get("/health")
def health():
    return {"ok": True, "service": "aprs", "v1_base": V1_BASE_URL}


@app.get("/config")
def config():
    return _get("/api/aprs/config")


@app.get("/messages")
def messages():
    return _get("/api/aprs/messages")


@app.post("/send")
def send(payload: Dict[str, Any] = Body(default={})):  # passthrough to v1
    return _post("/api/aprs/send", payload)


@app.post("/config")
def update_config(payload: Dict[str, Any] = Body(default={})):  # passthrough to v1
    return _post("/api/aprs/config", payload)
